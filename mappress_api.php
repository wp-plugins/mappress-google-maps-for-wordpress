...
class Mappress_Poi extends Mappress_Obj {
	...
	*/
	function geocode($auto=true) {
		// Only geocode if point lat/lng are not defined
		...
		if (!$response)
			return new WP_Error('geocode', sprintf(__('Invalid geocoding response: %s', 'mappress'), $response));

		//Decode response and automatically use first address
		$response = json_decode($response['body']);

		if (!$response  || !isset($response->results) || !isset($response->results[0]))
			return new WP_Error('geocode', sprintf(__("No Google geocoding result for address: %s", 'mappress'), $this->address));
		...
		// Parse out title & body
		$this->correctedAddress = str_replace(', USA', '', $this->correctedAddress);
		$comma = strpos($this->correctedAddress, ",");
		if ($comma > 0) {
			$this->title = substr($this->correctedAddress, 0, $comma);
			$this->body = substr($this->correctedAddress, $comma + 2);
		} else {
