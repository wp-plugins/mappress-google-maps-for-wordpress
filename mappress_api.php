...
class Mappress_Options extends Mappress_Obj {
	...
	// $args = null : retrieve all current values
	// $args = array : retrieve all current values, overriding with the ones in the array
	function get($atts = null) {
	...
	function save() {
		update_option('mappress_options', $this);
	}
} // End class Mappress_Options


class Mappress_Icon extends Mappress_Obj {
	...
	var $iconid = null,
		...
		$scaledSize = array('width' => 0, 'height' => '0');
...
class Mappress_Map extends Mappress_Obj {
	...
	*/
	...
	function display($atts = null) {
		...
		$html = "<div id='$options->mapName' class='mapp-map' style='width: $width; height: $height;' ><span class='mapp-loading'></span></div>";
						...
						. "<table>"
							...
							. "<tr>"
								. "<td colspan='2' align='left'>"
									. "<b>" . __('Directions', 'mappress') . "</b>"
									. "<span style='float:right'><a href='#' id='{$options->mapName}_closedirections'>" . __('Close', 'mappress') . "</a></span>"
								. "</td>"
							. "</tr>"
							. "<tr>"
								. "<td colspan='2'>"
							...
							. "</tr>"
							. "<tr>"
								. "<td><span class='mapp-b' title='" . __('End', 'mappress') . "'/></td>"
							...
							. "</tr>"
						. "</table>"
						. "<input type='button' value='" . __('Get Directions', 'mappress'). "' id='{$options->mapName}_get_directions' />"
	...
	function edit($maps = null, $postid) {
		...
		$options->navigationControlOptions = 0;
			...
			<table style='width:100%'>
				<tr>
					<td colspan="2" style='border-bottom:1px solid black'>
						<div>
							<input id='mapp_paypal' type='image' src='<?php echo plugins_url('images/btn_donate_LG.gif', __FILE__);?>' name='donate' alt='PayPal - The safer, easier way to pay online!' />
						...
						<div id='mapp_add_panel'>
							<p>
								...
								<span id='mapp_add_latlng'>
									...
									<input type='text' size='6' id='mapp_lat' value='' />
									<input type='text' size='6' id='mapp_lng' value='' />
								</span>
				...
				<tr>
					<td valign="top">
						<div id='mapp_maplist_panel'>
							<p>
								<b><?php _e('Maps in this post', 'mappress')?></b>
								<input class='button-primary' type='button' id='mapp_create_btn' value='<?php _e('Create Map', 'mappress')?>' />
							</p>

							<div id='mapp_maplist'></div>
						</div>

						<div id='mapp_adjust_panel'>
							<div id='mapp_adjust'>
								<p>
									<b><?php _e('Map ID')?>: </b><span id='mapp_mapid'></span>
								</p>
								<p>
									<b><?php _e('Title')?>: </b><input id='mapp_title' type='text' size='25' />
								</p>
								<p>
					...
					<td id='mapp_preview_panel' valign='top'>
						<div id='<?php echo $options->mapName?>' class='mapp-edit-map' ></div>
					</td>
				</tr>
			</table>
		<?php
	...
	function _load($options) {
		...
		$url = ($mappress->debug) ? "http://localhost/wordpress/wp-content/plugins/mappress-google-maps-for-wordpress" : plugins_url('', __FILE__);
		...
		echo "<script type='text/javascript' src='http://www.google.com/jsapi'></script>"
			...
			. "var mappIcons = new Array();"
			. "var mappLoader = new MappLoader(" . json_encode(array($options->language, $options->country)) . ");"
		...
		if ($options->customCSS) {
			echo "var s = document.createElement('LINK'); s.rel = 'stylesheet'; s.type = 'text/css';"
			. "s.href = '$url/css/custom.css?version=$mappress->version'; document.getElementsByTagName('head').item(0).appendChild(s);";
		}
