...
class Mappress_Options extends Mappress_Obj {
	var $directions = 'inline',                             // inline | google | none
        ...
        $border = null,      // array('style', 'color', 'width')
        $demoMap = true
        ;
...
class Mappress_Map extends Mappress_Obj {
	...
	*/
	function display($atts = null) {
		...
		// Set styles for container and canvas
		$container_style = "width:$width; ";      // Container has width (to allow centering) but no height (since it may expand)
        ...
        if ($options->border && isset($options->border['style']) && !empty($options->border['style'])) 
            $canvas_style .= sprintf("border: %s %s %s; ", $options->border['width'], $options->border['style'], $options->border['color']);
		...
		$html = "<div class='mapp-container' style='$container_style'>"
            . "<div id='$options->mapName' class='mapp-canvas' style='$canvas_style'></div>";

		// List of locations
		if ($options->poiList) {
			$html .= "<div id='{$options->mapName}_poi_list' class='mapp-poi-list' style='$poi_list_style'></div>";
		}
