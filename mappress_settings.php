...
class Mappress_Options extends Mappress_Obj {
	var $alignment = 'default',
		...
		$mapTypeIds,   // Map types for control: null=defaults, array ()=nothing, or custom style: array('roadmap', 'My Style')
		$marker_link = true,        // Link pois to underlying post for mashups
		$maxZoom,
		$minZoom,
		$metaKey,                   // Deprecated, left temporarily for back-compat
		$metaKeyAddress = array(),  // Array ('city', 'state', 'zip')
		$metaKeyLatLng = array(),   // Array (lat_field, lng_field)
		$metaKeyErrors,
		$metaSyncSave = true,
		$metaSyncUpdate = true,
		$name,
...
class Mappress_Settings {
	...
	function admin_init() {
		...
		add_settings_section('geocoding_settings', __('Geocoding', 'mappress'), array(&$this, 'section_settings'), 'mappress');
		add_settings_field('metaKeyAddress', __('Address fields', 'mappress'), array(&$this, 'set_pro'), 'mappress', 'geocoding_settings');
		add_settings_field('metaKeyErrors', __('Errors', 'mappress'), array(&$this, 'set_pro'), 'mappress', 'geocoding_settings');
