...
//		$options = array('')
	...
	function set_post_types() {
		...
		echo sprintf(__("Mark the %s to use MapPress:", "mappress"), $codex_link) . "<br/>";
	...
	function set_custom_css() {
		...
		echo "<input type='checkbox' name='mappress_options[customCSS]' $checked />";
			echo sprintf(__(" Include your own CSS.  You must create a file named %s in the %s directory", 'mappress'), "<code>custom.css</code>", "<code>/mappress/css</code>");
	}
	...
	function set_metafield() {
		...
		$code_link = '<code>[mappress source="custom"]</code>';
		...
		printf(__('Map addresses from a %s using the shortcode %s.', 'mappress'), $codex_link, $code_link);
	}
