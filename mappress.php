...
class Mappress {
	...
	function mappress()  {
		...
		add_action('wp_ajax_mapp_get', array(&$this, 'ajax_get'));
		add_action('wp_ajax_mapp_get_post_map_list', array(&$this, 'ajax_get_post_map_list'));
	...
	function ajax_delete() {
		...
		// If map is already deleted then return without error
	...
	function ajax_create() {
		...
		// Create a new map and save it to the database immediately
		$map = new Mappress_Map();
		$map->title = 'Untitled';
		$result = $map->save($postid);

		if (!$result)
			$this->ajax_response(__("Internal error when creating map for post ID '$postid'!", 'mappress'));
		else
	...
	function ajax_get() {
		...
		if (!$map)
			$this->ajax_response(__("Internal error when getting map ID '$mapid'!", 'mappress'));
		else
			$this->ajax_response('OK', array('map' => $map));
	}

	function ajax_get_post_map_list() {
		$postid = (isset($_GET['postid'])) ? $_GET['postid'] : null;

		$result = Mappress_Map::get_post_map_list($postid);
		if ($result === false)
			$this->ajax_response(__("Internal error getting map list for post ID '$postid'!"), 'mappress');

		$maps = $result;
		$this->ajax_response('OK', array('maps' => $maps));
	}
	...
	*/
	...
	function set_options($input) {
		...
		$options = new Mappress_Options($input);
		return $options;
	}
	...
	function set_post_types() {
		...
		$codex_link = "<a href='http://codex.wordpress.org/Custom_Post_Types'>" . __('custom post types', 'mappress') . "</a>";

		if (empty($all_post_types)) {
			echo sprintf(__("Your blog doesn't have any %s, but if you use them in the future you can set which ones should include MapPress here.", "mappress"), $codex_link);
			return;
		}
