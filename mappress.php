...
class Mappress {
	...
	*/
	function shortcode_map($atts='') {
		...
		if ($mapid) {
			...
			if (!$map)
				return;
		// If meta field was provided
		} else if (isset($meta_key) && class_exists('Mappress_Pro')) {
			$map = new Mappress_Map();
			$result = $this->get_meta_pois($post->ID, $meta_key);
			$map->pois = $result->pois;

			// Turn on autocenter unless it was turned off or a center was provided
			if (!isset($atts['autocenter']) && !isset($atts['center']))
				$atts['autocenter'] = true;
		} else {
			// No mapid, no meta field... try to just show the first map attached to the post
			$maps = Mappress_Map::get_post_map_list($post->ID);
			if (empty($maps))
				return;
			$map = $maps[0];
	...
	*/
	...
	function set_force_resize() {
		echo __(' Sized your maps wrong?  Click to permanently resize ALL of your maps: ', 'mappress');
		echo "<input type='text' size='2' name='force_resize_width' value='' /> x <input type='text' size='2' name='force_resize_height' value='' /> ";
		echo "<input type='submit' name='force_resize' value='" . __('Force Resize') . "' />";
	}
