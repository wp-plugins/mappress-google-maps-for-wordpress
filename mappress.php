...
class Mappress {
	...
	*/
	...
	function load($type = '') {
		...
		$js = (self::$dev) ? "http://localhost/dev/wp-content/plugins/mappress-google-maps-for-wordpress/src" : self::$baseurl . '/js';

		$version = Mappress::VERSION;

		if (self::$dev) {
			if ($type == 'editor') {
				wp_enqueue_script("mappress-gmaps", "https://maps.googleapis.com/maps/api/js?sensor=true&libraries=drawing", null, null, true);
				wp_enqueue_script('mappress_editor', "$js/mappress_editor.js", array('jquery', 'jquery-ui-core', 'jquery-ui-sortable'), $version, true);
			} else {
				wp_enqueue_script("mappress-gmaps", "https://maps.googleapis.com/maps/api/js?sensor=true", null, null, true);
			}
			wp_enqueue_script('mappress', "$js/mappress.js", array('jquery', 'jquery-ui-core'), $version, true);
			...
			wp_enqueue_script('mappress', "$js/mappress.min.js", array('jquery', 'jquery-ui-core'), $version, true);
		}
