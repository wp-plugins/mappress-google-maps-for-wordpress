...
class mappress {
	...
	var $map_defaults = array ('icons_url' => '', 'api_key' => '', 'country' => '', 'size' => 'MEDIUM', 'width' => 0, 'height' => 0, 'zoom' => 0, 'center_lat' => 0, 'center_lng' => 0,
								'address_format' => 'CORRECTED', 'bigzoom' => 1, 'googlebar' => 0, 'auto_center' => 1, 'scrollwheel_zoom' => 0, 'language' => '',
	...
	*/
	function hook_print_scripts() {
		...
		elseif ($this->development)
            ...
            $script = plugins_url('mappress-min.js', __FILE__);
	...
	*/
	function hook_admin_print_scripts() {
		...
		if ($this->development) {
			...
			wp_enqueue_script('mappress', plugin_urls('mappress.js', __FILE__), array('jquery-ui-core', 'jquery-ui-dialog'));
		} else {
			wp_enqueue_script('mappress_admin', plugins_url('mappress_admin-min.js', __FILE__), array('jquery-ui-core', 'jquery-ui-dialog'));
			wp_enqueue_script('mappress', plugins_url('mappress-min.js', __FILE__), array('jquery-ui-core', 'jquery-ui-dialog'));
		}
	...
	*/
	...
	function output_map_sizes($selected = "", $width = 0, $height = 0) {
		...
		echo "<input type='radio' name='mapp_size' id='mapp_size' value='CUSTOM' $checked />" . __('Custom', 'mappress');
	...
	function map($map, $pois, $editable = false) {
		...
		if ($editable) {
			...
			?>
				...
				<div>
					...
					<?php _e('Automatically center/zoom map when saved (uncheck to display map exactly as shown below)', 'mappress') ?>

					<p class="submit" style="padding: 0; float: none">
						<input type="button" id="mapp_recenter" value="<?php _e('Center/zoom now', 'mappress'); ?>" />
					</p>
					<br />
		...
		// Display maps
			...
			if ($map_args['streetview']) {
				...
				$map .= "<div style='float:right'><a href='#' onclick='{$map_name}.streetviewClose(); return false;'>" . __('Close', 'mappress') . "</a></div>";
				$map .= "<br />";
