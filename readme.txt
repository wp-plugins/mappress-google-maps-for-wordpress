...
== Upgrade Notice 2.38.1 ==
- When the plugin is activated it will create a directory in the WordPress uploads directory for custom icons.  You must move any custom icons to this directory:
`/wp-content/uploads/mappress/icons`

- The map controls settings have been expanded and changed; please check your settings and verify that your shortcodes are using the new control names!

- The map infoWindow (bubble) HTML and CSS has been simplified.  If you are using custom CSS please check the mappress.css file for the new settings.
...
2.38.2
=
* [MapPress Pro](http://wphostreviews.com/mappress) Added: templates are now available to change the map layout, poi list, and poi contents
* [MapPress Pro](http://wphostreviews.com/mappress) Added: a new dataTable option displays the poi list as a dataTable; use [mappress dataTable="true"] or specify an array of settings using PHP
* [MapPress Pro](http://wphostreviews.com/mappress) Added: mashup pois now display featured image thumbnails by default.  Use 'thumbs="false"' to turn this off.
* [MapPress Pro](http://wphostreviews.com/mappress) Added: parameter 'thumbSize' can be used to specify a size name, or 'thumbWidth' and 'thumbHeight' to force a thumbnail size
* [MapPress Pro](http://wphostreviews.com/mappress) Added: custom map styles are now supported in the settings screen
* [MapPress Pro](http://wphostreviews.com/mappress) Added: a setting and parameter 'style' are available to set the default maps to use a custom style
* [MapPress Pro](http://wphostreviews.com/mappress) Added: a setting and parameter 'mapTypeIds' are used to define which map types (including custom styles) are available
...
* [MapPress Pro](http://wphostreviews.com/mappress) Changed: mashup parameters 'marker_title' and 'marker_body' have been removed.  Use 'marker_link' instead (see below).
* [MapPress Pro](http://wphostreviews.com/mappress) Changed: if 'marker_link="true"' (the default) each poi will display the underlying post's thumbnail, excerpt and permalink.  If "false" the poi title and body will display as saved in the poi.
* [MapPress Pro](http://wphostreviews.com/mappress) Changed: widget parameters for 'marker_title' and 'marker_body' have been removed.
* [MapPress Pro](http://wphostreviews.com/mappress) Changed: mashup parameter 'show' and 'show_query' are deprecated.  Use 'query' instead.
* [MapPress Pro](http://wphostreviews.com/mappress) Changed: use parameter 'query' for mashup queries.  Specify an actual query or use "current" for current posts or "all" for all posts with maps.  For example: [mashup query="all"]
* [MapPress Pro](http://wphostreviews.com/mappress) Changed: parameter 'show_query' is now just 'query'.  Provide a query or leave it blank to map the currently displayed blog posts.
* [MapPress Pro](http://wphostreviews.com/mappress) Changed: parameter "map=true" is appended to all mashup queries, which optimizes the results to include only posts with maps
